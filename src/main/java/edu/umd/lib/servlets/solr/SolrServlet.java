 ...
 */
package edu.umd.lib.servlets.solr;
...
public class SolrServlet extends HttpServlet {
  ...
  private static final Logger log = LoggerFactory.getLogger(SolrServlet.class);
  ...
  public SolrServlet() {
    ...
    exchangeCamelContext();
  }

  public void exchangeCamelContext() {
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
    ...
    try {
      ...
      String solrAction = req.getParameter("solrAction");
      templateParams.put("formAction", formAction);
      templateParams.put("solrAction", solrAction);
      ...
      if ((param = req.getParameter("xpath")) != null || (param = req.getParameter("sql")) != null) {
        ...
        templateParams.put("queryResultTotalSize", nodeIterator.getTotalSize());

        long i = 0;
        if ((formAction.equals("Index")) && (queryResult != null)) {

          RowIterator rowIterator = queryResult.getRows();
          i = rowIterator.getSize();
          while (rowIterator.hasNext()) {
            Node n = rowIterator.nextRow().getNode();
            Boolean success = toCamel(n.getUUID(), solrAction);
            System.out.println(n.getUUID());
            System.out.println(n.getName());
          }
        }
        templateParams.put("nodeCount", i);
  ...
  /**
   * Take the UUIDs and send them to Camel
   *
   * @param uuid
   */
  protected boolean toCamel(String uuid, String solrAction) throws Exception {
    JSONObject doc = new JSONObject();
    // doc.put("handleUuid", uuid);
    doc.put("action", solrAction);
    // doc.put("methodName", solrAction);
    doc.put("subjectId", uuid);

    ProducerTemplate producer = camelContext.createProducerTemplate();

    producer.sendBody("file:inbox?autoCreate=true&charset=utf-8", doc.toString());
    return true;
  }
